...
module SmrfClient
  class SummaryPost200ResponseOneOf1
    ...
    def self.openapi_all_of
      ...
      :'SummaryPost200ResponseOneOf1AllOf'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SmrfClient::SummaryPost200ResponseOneOf1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SmrfClient::SummaryPost200ResponseOneOf1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'counties')
        self.counties = attributes[:'counties']
      end
    ...
    def hash
      [counties, npi_count, count, maximum_rate, median_rate, minimum_rate, q1, q3].hash
    end
